...
function cone(m: THREE.Material, r: number, h: number): THREE.Mesh { return new THREE.Mesh(new THREE.ConeGeometry(r, h, 16, 3), m); }
...
function hair(head: THREE.Group, m: Mats, a: Archetype, g: Gender, r: number): void {
  const h = sphere(m.hair, r * 1.04, 18, 10); h.scale.set(1.04, 0.62, 1.02); add(head, h, 0, r * 0.3, -0.015);
  if (g === "f" || a === "mara" || a === "dana" || a === "priya") {
    const bang = capsule(m.hair, r * 0.18, r * 0.65, 10, 3); bang.rotation.z = Math.PI / 2; add(head, bang, 0, r * 0.42, r * 0.72);
    const l = capsule(m.hair, r * 0.12, r * 0.5, 9, 3); const rr = capsule(m.hair, r * 0.12, r * 0.5, 9, 3); l.rotation.z = 0.14; rr.rotation.z = -0.14;
    add(head, l, -r * 0.83, -r * 0.05, r * 0.13); add(head, rr, r * 0.83, -r * 0.05, r * 0.13);
  }
  if (a === "crosby") { const hood = sphere(m.shirt, r * 1.22, 18, 10); hood.scale.set(1.04, 0.92, 1.04); add(head, hood, 0, 0, -r * 0.12); }
  if (a === "magician") { add(head, new THREE.Mesh(new THREE.CylinderGeometry(r * 1.25, r * 1.25, 0.08, 20), m.accent), 0, r * 0.86, 0); add(head, cone(m.shirt, r * 0.65, r), 0, r * 1.22, 0); }
  if (a === "worker") { const helm = sphere(m.accent, r * 1.16, 18, 10); helm.scale.set(1.05, 0.42, 1.05); add(head, helm, 0, r * 0.77, 0); add(head, box(m.accent, r * 1.35, 0.06, r * 0.55), 0, r * 0.67, r * 0.42); }
}
